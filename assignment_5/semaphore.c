...
static __init int pchar_init(void)
{
    ...
    sema_init(&s,1);
    printk(KERN_INFO "%s: semaphore_init() initialized semaphore lock for all devices.\n", THIS_MODULE->name);
...
static int pchar_open(struct inode *pinode, struct file *pfile)
{
	...
	down(&s);
    printk(KERN_INFO "%s: semaphore is locked.\n", THIS_MODULE->name);
    return 0;
}
...
static int pchar_close(struct inode *pinode, struct file *pfile)
{
    ...
    up(&s);
    printk(KERN_INFO "%s: semaphore is unlocked.\n",THIS_MODULE->name);
    	return 0;
}
