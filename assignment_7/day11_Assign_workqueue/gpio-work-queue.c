...
static __init int GPIO_init(void)
{
	...
	pclass = class_create(THIS_MODULE, "GPIO_class");
	if (IS_ERR(pclass))
	{
	...
	pdevice = device_create(pclass, NULL, devno, NULL, "GPIO_device_%d", 0);
	if (IS_ERR(pdevice))
	{
	...
	ret = gpio_request(SWITCH_GPIO, "SWITCH");
	if (ret != 0)
	{
	...
	printk(KERN_INFO "%s : GPIO_init() is finished\n", THIS_MODULE->name);
...
static __exit void GPIO_exit(void)
{
	...
	printk(KERN_INFO "%s : GPIO_exit() is called \n", THIS_MODULE->name);
	...
	printk(KERN_INFO "%s : GPIO_exit is finished\n", THIS_MODULE->name);
}
...
static ssize_t GPIO_read(struct file *pfile, char __user *ubuf, size_t size, loff_t *poffset)
{
